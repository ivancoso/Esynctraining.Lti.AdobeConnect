...
select 
  ...
  q.submoduleItemId as subModuleItemId, ac.dateCreated, ac.includeACEmails as includeAcEmails, cat.categoryName, um.acUserModeId, 
   COUNT(QR.quizResultId) AS totalParticipants, 
       ...
       FROM QuizResult QR
       WHERE QR.score > 0 AND ac.acSessionId = QR.acSessionId) AS activeParticipants,
       (SELECT SUM(score)from QuizResult where acSessionId = ac.acSessionId ) AS TotalScore,          
	   --q.isPostQuiz,
	   q.quizName,
    ac.userId
