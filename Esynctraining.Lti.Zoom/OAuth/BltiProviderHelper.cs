...
namespace Esynctraining.Lti.Zoom.OAuth
{
    ...
    public sealed class BltiProviderHelper
    {
        ...
        public static bool VerifyBltiRequest(LmsCompany credentials, HttpRequest request, Func<bool> validateLmsCaller)
        {
            ...
            //var form = new FormCollection(request.Form);
            //if (form == null)
            //{
            ...
            string nonce = request.Form["oauth_nonce"];
            if (nonce == null)
            {
            ...
            string normalizedUrl = string.Format("{0}://{1}", schema, request.GetUri().Host);
            if (
                !((schema == "http" && request.GetUri().Port == 80)
                  || (schema == "https" && request.GetUri().Port == 443)
                  || (schema == "https" && request.GetUri().Port == 80 && request.GetUri().Scheme == "http") // TRICK: using "X-Forwarded-Proto" proxy
                  ))
            {
                normalizedUrl += ":" + request.GetUri().Port;
            }

            normalizedUrl += request.GetUri().AbsolutePath;
            ...
            string consumerKey = request.Form["oauth_consumer_key"];
