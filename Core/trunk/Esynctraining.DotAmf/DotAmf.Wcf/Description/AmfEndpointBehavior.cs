// --------------------------------------------------------------------------------------------------------------------
// <copyright company="" file="AmfEndpointBehavior.cs">
//   
// </copyright>
// <summary>
...
namespace DotAmf.ServiceModel.Description
{
    ...
    ///     Enables the AMF support for an endpoint.
    /// </summary>
    public sealed class AmfEndpointBehavior : IEndpointBehavior
    {
        ...
        /// <summary>
        /// Implements the <c>IEndpointBehavior.ApplyDispatchBehavior</c> method to support modification or extension of the
        ///     client across an endpoint.
        /// </summary>
        /// <param name="endpoint">
        ...
        public void ApplyDispatchBehavior(ServiceEndpoint endpoint, EndpointDispatcher endpointDispatcher)
        {
            ...
            bool amfErrorHandlingEnabled;
            if (ConfigurationManager.AppSettings["EnableDotAMFBuiltInErrorHandling"] == null
                || !bool.TryParse(
                    ConfigurationManager.AppSettings["EnableDotAMFBuiltInErrorHandling"], 
                    out amfErrorHandlingEnabled) || amfErrorHandlingEnabled)
            {
            ...
            foreach (OperationDescription descriptor in endpoint.Contract.Operations)
            {
                if (descriptor.Behaviors.OfType<AmfOperationBehavior>().FirstOrDefault() != null)
                {
                    continue;
                }
            ...
            // Command operation
            var commandOperation = new DispatchOperation(
                endpointDispatcher.DispatchRuntime, 
                AmfOperationKind.Command, 
                AmfOperationKind.Command, 
                null) {
                         Invoker = new AmfCommandInvoker(capabilities), Formatter = new AmfCommandFormatter(), 
                      };
            endpointDispatcher.DispatchRuntime.Operations.Add(commandOperation);

            // Fault operation
            var faultOperation = new DispatchOperation(
                endpointDispatcher.DispatchRuntime, 
                AmfOperationKind.Fault, 
                AmfOperationKind.Fault, 
                null) {
                         Invoker = new AmfFaultInvoker(capabilities), Formatter = new AmfGenericOperationFormatter() 
                      };
