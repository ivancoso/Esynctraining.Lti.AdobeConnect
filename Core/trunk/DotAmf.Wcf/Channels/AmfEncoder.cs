// --------------------------------------------------------------------------------------------------------------------
// <copyright company="" file="AmfEncoder.cs">
//   
// </copyright>
// <summary>
...
namespace DotAmf.ServiceModel.Channels
{
    ...
    /// </summary>
    /// <remarks>
    ///     The encoder is the component that is used to write
    ///     messages to a stream and to read messages from a stream.
    /// </remarks>
    internal sealed class AmfEncoder : MessageEncoder
    {
        #region Constants
        ...
        public override string MediaType
        {
            get
            {
                return AmfEncoderFactory.DefaultAmfMediaType;
            }
        }
        ...
        /// <summary>
        /// Returns a value that indicates whether a specified message-level content-type value is supported by the message
        ///     encoder.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// <c>true</c> if the message-level content-type specified is supported; otherwise <c>false</c>.
        /// </returns>
        public override bool IsContentTypeSupported(string contentType)
        {
            // A perfect match
            if (contentType == this.ContentType || contentType == WebContentType)
            {
                return true;
            }

            // Not a perfect match, but still OK
            if (this.MediaType != null && contentType.Length == this.MediaType.Length
                && contentType.Equals(this.MediaType, StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        ...
        /// BufferManager that manages the buffer from which the message is deserialized.
        /// </param>
        /// <param name="contentType">
        ...
        /// </remarks>
        /// <param name="stream">
        /// Stream object from which the message is read.
        /// </param>
        ...
        /// The offset of the segment that begins from the start of the byte array that provides the
        ///     buffer.
        /// </param>
        /// <returns>
        /// The buffer to which the message is serialized.
        /// </returns>
            ...
            int messageOffset)
        {
            ...
            {
                this.WriteMessage(message, ms);

                messageBytes = ms.ToArray();
                messageLength = messageBytes.Length;
            }

            byte[] totalBytes = bufferManager.TakeBuffer(messageLength + messageOffset);
        ...
        public override void WriteMessage(Message message, Stream stream)
        {
            try
            {
                ...
                {
                    // Read AMFX message
                    XmlDictionaryWriter writer = XmlDictionaryWriter.CreateDictionaryWriter(AmfxWriter.Create(ms));
