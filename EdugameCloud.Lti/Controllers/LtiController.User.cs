    ...
    public partial class LtiController : Controller
    {
        ...
        public virtual ActionResult UpdateUser(string lmsProviderName, LmsUserDTO[] users, int meetingId)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(error))
                    return Json(OperationResultWithData<LmsUserDTO[]>.Success(updatedUsers.ToArray()));
        ...
        public virtual ActionResult RemoveFromAcMeeting(string lmsProviderName, int meetingId, LmsUserDTO[] users)
        {
            ...
            foreach (var user in users)
            {
                ...
                {
                    error += "\n" + GetOutputErrorMessage("UpdateUser", credentials, ex);
                }
            }

            if (string.IsNullOrEmpty(error))
            {
                return Json(OperationResultWithData<string[]>.Success(removedIds.ToArray()));
            }
            return Json(OperationResult.Error(error));
        }
