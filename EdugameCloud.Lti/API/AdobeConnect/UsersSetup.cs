namespace EdugameCloud.Lti.API.AdobeConnect
{
    ...
    public class UsersSetup
    {
        ...
        public bool AreUsersSynched(
            ...
            AdobeConnectProvider provider)
        {
            if (!this.IsTeacher(param) || meeting.LmsMeetingType != (int)LmsMeetingType.Meeting)
            {
                return true;
            }

            string error;
            var lmsUsers = this.GetLMSUsers(companyLms, meeting, lmsUserId, courseId, out error, param);
            ...
            foreach (var lmsUser in lmsUsers)
            {
                ...
                if (user.PrincipalId == null || !principalIds.Contains(user.PrincipalId))
                {
                    return false;
                }
