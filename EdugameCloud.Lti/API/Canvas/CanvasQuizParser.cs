    ...
    public class CanvasQuizParser
    {
        ...
        private static string ExtractFilesFromText(string text, ref int fileIndexStart, Dictionary<int, LmsQuestionFileDTO> fileIds)
        {
            ...
            foreach (XmlNode img in images)
            {
                ...
                {
                    int.TryParse(img.Attributes["width"].Return(t => t.Value, string.Empty) ?? string.Empty, out width);
                    int.TryParse(img.Attributes["height"].Return(t => t.Value, string.Empty) ?? string.Empty, out height);
                    
                    fileIds.Add(
                        fileIndexStart,
                        new LmsQuestionFileDTO()
                            {
                                fileName = img.Attributes["alt"].Return(t => t.Value, string.Empty),
                                fileUrl = img.Attributes["src"].Return(t => t.Value, string.Empty),
                                width = width,
                                height = height
                            });
