...
namespace EdugameCloud.Lti.Zoom.Api.Host.Controllers
{
    ...
    public class RecordingsController : BaseApiController
    {
        ...
        public RecordingsController(
            ...
            ILogger logger, ICache cache, LmsCourseMeetingModel lmsCourseMeetingModel, ZoomRecordingService recordingService)
        ...
        public virtual async Task<OperationResultWithData<IEnumerable<ZoomRecordingSessionDto>>> GetRecordings(int meetingId)
        {
            ...
            var recordings = _recordingService.GetRecordings(meeting.AudioProfileId, meeting.ScoId);
        ...
        public virtual async Task<OperationResultWithData<IEnumerable<ZoomRecordingsTrashItemDto>>> GetTrashRecordings(int meetingId)
        {
            ...
            var recordings = _recordingService.GetTrashRecordings(meeting.AudioProfileId, meeting.ScoId);
            return recordings.ToSuccessResult();
        }
        ...
        public virtual async Task<OperationResult> DeleteRecording(int meetingId, string recordingFileId, [FromQuery]bool trash = true ) 
        {
            ...
            var meetingSessionId =
                _recordingService.GetRecordingFileSessionId(userId, meeting.ScoId, recordingFileId, trash);
        ...
        [Microsoft.AspNetCore.Mvc.HttpDelete]
        [LmsAuthorizeBase(ApiCallEnabled = true)]
        public virtual async Task<OperationResult> DeleteAllRecordings(int meetingId, string recordingId, [FromQuery]bool trash = false) //this method is called only from trash, so no need to set trash=true by default
        {
            try
            {
                var meeting = _lmsCourseMeetingModel.GetOneByCourseAndId(LmsCompany.Id, CourseId, meetingId);
        ...
        public virtual async Task<OperationResult> RecoverRecording(int meetingId, string recordingFileId)
        {
            ...
            var meetingSessionId =
                _recordingService.GetRecordingFileSessionId(userId, meeting.ScoId, recordingFileId, true);
            if (meetingSessionId == null)
            {
