...
{
    /// <summary>
    /// report template for Goddard
    /// </summary>
    public class GoddardExtendedReportService : IExtendedReportService
    {
                                    ...
                                    if (qqr != null)
                                    {
                                        switch ((QuestionTypeEnum)q.QuestionType.Id)
                                        {
                                            ...
                                            case QuestionTypeEnum.SingleMultipleChoiceText:
                                                ...
                                                break;
                                            case QuestionTypeEnum.TrueFalse:
                                                var correctRow = correctRows.First(); //single
                                                var rowToMark = qqr.IsCorrect ? correctRow :
                                                    (endRow - 2 == correctRow ? correctRow - 1 : correctRow + 1); //position related to Score row
                                                ws.Cells[rowToMark, startUserPartColumn].Value = "X";
                                                break;
                                        }
                                    }
                                    else
                                    {
                                        ws.Cells[endRow - 2, startUserPartColumn].Value = "X"; // No Answer
                                    }

                                    ws.Cells[endRow - 1, startUserPartColumn].Value = (qqr!= null && qqr.IsCorrect) ? 1 : 0;
