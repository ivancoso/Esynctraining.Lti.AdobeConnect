    ...
    public class SessionsController : BaseApiController
    {
        ...
        public async Task<OperationResultWithData<IEnumerable<MeetingSessionDto>>> CreateBatch(int meetingId, [FromBody]CreateMeetingSessionsBatchDto dto)
        {
            try
            {
                ...
                var result = await _meetingSessionService.CreateBatchAsync(dto, meeting, CourseId, lmsSettings, Param);
        ...
        public async Task<OperationResultWithData<MeetingSessionDto>> CreateSession(int meetingId)
        {
            try
            {
                ...
                var lmsSettings = LmsLicense.GetLMSSettings(Session);
                var session = await _meetingSessionService.CreateSessionAsync(meeting, lmsSettings, Param);
