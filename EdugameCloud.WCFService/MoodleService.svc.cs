...
namespace EdugameCloud.WCFService
{
    ...
    public class MoodleService : BaseService, IMoodleService
    {
        ...
        /// </returns>
        public QuizesAndSubModuleItemsDTO ConvertQuizzes(LmsQuizConvertDTO quizesInfo)
        {
            ...
            if (moodleUser == null)
            {
                var error = new Error(Errors.CODE_ERRORTYPE_GENERIC_ERROR, "TokenNotFound", "No user details were found");
                this.LogError("Moodle.ConvertQuizzes", error);
            ...
            foreach (var id in quizesInfo.quizIds)
            {
                ...
                string errorMessage = string.Empty, errorString = string.Empty;
                var q = MoodleQuizParser.Parse(xmlDoc, ref errorMessage, ref errorString);

                if (q == null)
                {
                    var error = new Error(Errors.CODE_ERRORTYPE_GENERIC_ERROR, errorMessage, errorString);
        ...
        /// </returns>
        public SurveysAndSubModuleItemsDTO ConvertSurveys(LmsQuizConvertDTO quizesInfo)
        {
            ...
            {
                var error = new Error(Errors.CODE_ERRORTYPE_GENERIC_ERROR, "TokenNotFound", "No user details were found");
                this.LogError("Moodle.ConvertSurveys", error);
                throw new FaultException<Error>(error, error.errorMessage);
            }
...
//            var subModuleItemsQuizes = new Dictionary<int, int>();
        ...
        /// </returns>
        public MoodleQuizInfoDTO GetQuizzesForUser(MoodleUserInfoDTO userInfo)
        {

            var token = userInfo.token ?? this.GetToken(userInfo.domain, userInfo.name, userInfo.password); 
        ...
        /// The <see cref="MoodleQuizInfoDTO"/>.
        /// </returns>
        public MoodleQuizInfoDTO GetSurveysForUser(MoodleUserInfoDTO userInfo)
        {
            var token = userInfo.token ?? this.GetToken(userInfo.domain, userInfo.name, userInfo.password);
        ...
        /// </returns>
        public LmsUserParametersDTO GetAuthenticationParametersById(string id)
        {

            var param = this.LmsUserParametersModel.GetOneByAcId(id).Value;
            var result = param != null ? new LmsUserParametersDTO(param) : null;
            ...
            if (result != null)
            {
                result.provider = "Moodle";
                result.domain = "http://64.27.12.60";
            }
        ...
        private string GetToken(string domain, string username, string password)
        {
            ...
            if (token.error != null)
            {
                var error = new Error(Errors.CODE_ERRORTYPE_GENERIC_ERROR, "TokenNotFound", token.error);
                this.LogError("AppletResult.GetById", error);
                throw new FaultException<Error>(error, error.errorMessage);
            }
