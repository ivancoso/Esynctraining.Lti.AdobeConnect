...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.odbc.update">
    ...
    <xsl:if test="$editable">
      const String SqlUpdate = @"Update <xsl:value-of select="codegen:quoteIdentifier(@wdm:Schema)" />.<xsl:value-of select="codegen:quoteIdentifier(@name)" /> Set
    ...
    public override <xsl:value-of select="$class-name" /> update(<xsl:value-of select="$class-name" /><xsl:text> </xsl:text><xsl:value-of select="$functionParam" />)
    {
      <xsl:if test="$editable">
        ...
        {
        using(OdbcCommand sqlCommand = Database.CreateCommand(SqlUpdate))
        {
        <xsl:for-each select="xs:complexType/xs:attribute[not(concat('@',@name) = key('pk',$table)/@xpath) and codegen:IsEditable(@type)]">
