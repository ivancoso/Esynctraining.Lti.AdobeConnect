package ui.recording
{
	...
	public class VideoRecordingController
	{
		...
		/**
		 * @private
		 * Instance to display recording video
		 */
		...
		private function initMediaDevices():void
		{
			...
			if (_camera)
			{
				...
				_videoRecord = new Video(Defaults.VIDEO_WIDTH, Defaults.VIDEO_HEIGHT);
				_videoRecord.attachCamera(_camera);
				_view.recordContainer.addChild(_videoRecord);
			}	
		}
		 ...
		 * @private
		 */
		private function releaseMediaDevices():void
		{
			if (_videoRecord)
			{
				_videoRecord.attachCamera(null);
				_view.recordContainer.removeChild(_videoRecord);
				_videoRecord = null;
			}
		...
		private function startRecording():void
		{
			...
			_netStreamRec.addEventListener(AsyncErrorEvent.ASYNC_ERROR, onAsyncError);
		...
		private function recordingStopped():void
		{
			...
			if (_netStreamRec)
			{
				...
				_netStreamRec.removeEventListener(AsyncErrorEvent.ASYNC_ERROR, onAsyncError);
				_netStreamRec.attachCamera(null);
				_netStreamRec.attachAudio(null);
			}
		}
		...
		private function startPlaying():void
		{
			...
			_netStreamPlayback.addEventListener(AsyncErrorEvent.ASYNC_ERROR, onAsyncError);
		...
		private function playingStopped():void
		{
			...
			if (_netStreamPlayback)
			{
				...
				_netStreamPlayback.removeEventListener(AsyncErrorEvent.ASYNC_ERROR, onAsyncError);
			}
		...
		private function onNetStatus(event:NetStatusEvent):void
		{
			switch (event.info.code)
			{
				...
				case "NetStream.Record.Failed":
					recordingStopped();
					new Alert().show("Attempt to record a stream failed.", _view, true);
					break;
				case "NetStream.Record.NoAccess":
					recordingStopped();
					new Alert().show("Attempt to record a stream that is still playing or the client has no access right.", _view, true);
					break;
				...
				case "NetStream.Play.StreamNotFound":
					new Alert().show("The file can't be found. You should record first.", _view, true);
					break;
				case "NetStream.Play.Failed":
					new Alert().show("Play failed", _view, true);
					break;
				...
				case "NetStream.Connect.Failed":
					new Alert().show("The P2P connection attempt failed.\n" + event.info.description, _view, true);
					break;
			}
		}
