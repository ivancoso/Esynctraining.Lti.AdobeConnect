...
public class ComplexTypeArgInfo extends ArgInfo {
    ...
    /**
    * Specifies if current ArgInfo have been instantiated
    */
	...
	/**
	 * Instantiates views and model for currecnt object.
	 */
	private void instantiate()
	{
		...
		for (Field field : fields) {
			if (Modifier.isPublic(field.getModifiers())) {
				...
				View mainView = fieldModel.getMainView();
				mainView.setVisibility( View.GONE );
				views.add( mainView );
			}
		}
		controller.addItems( views.toArray( new View[ views.size() ] ) );
		instantiated = true;
	  }
