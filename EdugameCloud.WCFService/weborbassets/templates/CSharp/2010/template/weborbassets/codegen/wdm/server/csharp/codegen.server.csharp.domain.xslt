...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.domain">
    ...
    <xsl:for-each select="xs:complexType/xs:attribute[not(concat('@',@name) = key('fk',$class-name)[@wdm:Schema=$schema]/@xpath)]">
      protected <xsl:value-of select="codegen:CSharpDataType(@type)"/><xsl:if test="@use = 'optional' and codegen:IsNullable(@type)">?</xsl:if><xsl:text> </xsl:text>_<xsl:value-of select="codegen:getFunctionParameter(codegen:getPropertyName($table, $schema, @name))" />;
    </xsl:for-each>
    ...
    <xsl:for-each select="xs:keyref">
      ...
      // parent tables
      protected <xsl:value-of select="codegen:getClassName($parent-table,$parent-schema)"/><xsl:text> </xsl:text><xsl:value-of select="$parent-property" />
      <!--xsl:if test="xs:field[substring(@xpath,2) = ../../xs:complexType/xs:attribute[@use='required']/@name]">
        = new <xsl:value-of select="codegen:getClassName($parent-table,$parent-schema)"/>()
      </xsl:if-->;
    </xsl:for-each>
    ...
    <xsl:for-each select="xs:complexType/xs:attribute">
      ...
      <xsl:value-of select="codegen:getFunctionParameter(codegen:getPropertyName($table, $schema, @name))" />
      <xsl:if test="position() != last()">,</xsl:if>
    </xsl:for-each>
    )
    {
    <xsl:for-each select="xs:complexType/xs:attribute">
      this.<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)" /> = <xsl:value-of select="codegen:getFunctionParameter(codegen:getPropertyName($table, $schema, @name))" />;
    </xsl:for-each>
    }
    ...
    public override bool contains(Hashtable fields)
    {
      int matchCount = 0;
      <xsl:for-each select="xs:complexType/xs:attribute">
        if(fields.ContainsKey("<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)"/>"))
        {
          <xsl:choose>
            <xsl:when test="@type = 'xs:anyURI'">
              if(! (new Guid(fields["<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)"/>"].ToString()).Equals( 
                this.<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)"/> )))
            </xsl:when>
            <xsl:otherwise>
              if(!fields["<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)"/>"].Equals(this.<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)"/>))
            </xsl:otherwise>
          </xsl:choose>
            return false;
    ...
    <xsl:for-each select="xs:complexType/xs:attribute">
      ...
      public <xsl:value-of select="codegen:CSharpDataType(@type)" /><xsl:if test="@use = 'optional' and $support-null">?</xsl:if><xsl:text> </xsl:text><xsl:value-of select="codegen:getPropertyName($table, $schema, @name)" />
      {
        <xsl:choose>
          <xsl:when test="not(concat('@',@name) = key('fkByElementId',$id)/@xpath)">
            get { return _<xsl:value-of select="codegen:getFunctionParameter(codegen:getPropertyName($table, $schema, @name))" />;}
            set 
            { 
                ...
                <xsl:if test="@xpath = concat('@',$attribute-name)">
                  ...
                  <xsl:choose>
                    <xsl:when test="$optional and $support-null">
                      ...
                      {
                      if(<xsl:value-of select="$parent-property"/> == null)
                        <xsl:value-of select="$parent-property"/> = new <xsl:value-of select="codegen:getClassName($parent-table-name,$parent-table-schema)"/>();

                        <xsl:value-of select="$parent-property"/>.<xsl:value-of select="codegen:getPropertyName($parent-table-name,$parent-table-schema,substring($parent-pk-field-name,2)) "/> = value.Value;
                      }
                    </xsl:when>
                    <xsl:otherwise>
                      if(<xsl:value-of select="$parent-property"/> == null)
    ...
    <xsl:for-each select="xs:keyref">
      ...
      public <xsl:value-of select="codegen:getClassName($parent-table,$parent-schema)"/><xsl:text> </xsl:text><xsl:value-of select="codegen:getParentProperty($table,$schema,$parent-table, $parent-schema, @name,0)" />
      {
    ...
    <xsl:value-of select="$class-name"/><xsl:text> </xsl:text><xsl:value-of select="codegen:getFunctionParameter($class-name)"/> = new <xsl:value-of select="$class-name"/>();
      ...
      <xsl:for-each select="xs:complexType/xs:attribute">
        <xsl:value-of select="codegen:getFunctionParameter($class-name)"/>.<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)" /> = this.<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)" />;
      </xsl:for-each>
        ...
        <xsl:variable name="child-property" select="concat('_',codegen:getChildProperty($table,$schema,@name,@wdm:Schema,$fk, 1))" />
          <xsl:choose>
            <xsl:when test="count(xs:key/xs:field[@xpath = key('fkByName',$fk)/@xpath]) = count(xs:key/xs:field)">
              // one to one relation
              <xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/><xsl:text> </xsl:text><xsl:value-of select="$child-property" />;
              public <xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/><xsl:text> </xsl:text><xsl:value-of select="codegen:getChildProperty($table,$schema,@name,@wdm:Schema, $fk, 0)" />
              {
            ...
            <xsl:otherwise>
              ...
              private List&lt;<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>&gt; <xsl:value-of select="$child-property" />;

              public List&lt;<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>&gt;<xsl:text> </xsl:text><xsl:value-of select="codegen:getChildProperty($table,$schema,@name,@wdm:Schema,$fk, 1)" />
              {
              get { return <xsl:value-of select="$child-property" />;}
              set { <xsl:value-of select="$child-property" /> = value; }
              }
              ...
              public <xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/><xsl:text> </xsl:text>add<xsl:value-of select="codegen:getChildProperty($table,$schema,@name,@wdm:Schema,$fk, 0)" />Item(
              <xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/><xsl:text> </xsl:text><xsl:value-of select="codegen:getFunctionParameter( codegen:getClassName(@name,@wdm:Schema) )"/>)
              {
              <xsl:value-of select="codegen:getFunctionParameter( codegen:getClassName(@name,@wdm:Schema) )"/>.<xsl:value-of select="codegen:getParentProperty(@name, @wdm:Schema, $table, $schema, $fk, 0)"/> = this;
