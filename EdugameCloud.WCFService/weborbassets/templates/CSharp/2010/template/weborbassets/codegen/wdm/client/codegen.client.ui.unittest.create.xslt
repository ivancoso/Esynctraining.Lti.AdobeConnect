  ...
  <xsl:template name="codegen.client.ui.unittest.create">
    ...
    <file name="UTest{$class-name}Create.as" override="true" addxmlversion="true">
      ...
      <xsl:for-each select="xs:complexType/xs:attribute[not(@msdata:AutoIncrement='true')]">
        ...
        <xsl:if test="not(concat('@',@name) = key('fkByElementId',$id)/@xpath)">
          <xsl:value-of select="codegen:getFunctionParameter($class-name)" />.<xsl:value-of select="codegen:getPropertyName($table, $schema, @name)"/> = <xsl:value-of select="codegen:getPrimitiveValue(@type)"/>;
        </xsl:if>
    </xsl:for-each>
    ...
    <xsl:for-each select="xs:keyref">
      ...
      <xsl:for-each select="key('table',@refer)">
        ActiveRecords.<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>.findFirst().addResponder(
      ...
      <xsl:for-each select="xs:keyref">
        <xsl:variable name="relationName" select="codegen:getParentProperty($table,$schema,key('table',@refer)/@name,key('table',@refer)/@wdm:Schema,@name,0)" />
        <xsl:for-each select="key('table',@refer)">
          protected function on<xsl:value-of select="$relationName" />Received(<xsl:value-of select="codegen:getFunctionParameter($relationName)"/>:<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>):void
          {
