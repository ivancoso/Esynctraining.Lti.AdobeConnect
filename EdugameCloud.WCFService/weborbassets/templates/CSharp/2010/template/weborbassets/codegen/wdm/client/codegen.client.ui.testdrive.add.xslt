      ...
      <mx:TitleWindow
        ...
        <mx:Script>
          ...
          private function onCreationComplete():void
          {
            PopUpManager.centerPopUp(this);
			 ...
			 <xsl:variable name="keyrefSchema" select="@wdm:Schema"/>
        ...
        <xsl:for-each select="xs:complexType/xs:attribute[not(@msdata:AutoIncrement='true') and not(codegen:IsBinary(@type))]">
          ...
          <mx:Binding>
            ...
            <xsl:attribute name="destination">_model.<xsl:value-of select="codegen:getProperty($table,$schema,@name)"/></xsl:attribute>
          </mx:Binding>
          </xsl:if>
        </xsl:for-each>
        <xsl:for-each select="xs:keyref">
			...
			<xsl:variable name="keyrefSchema" select="@wdm:Schema"/>
				<xsl:for-each select="key('table',@refer)">
					<xsl:if test="@wdm:Schema = $keyrefSchema">
						<xsl:variable name="relationName" select="codegen:getParentProperty($table, $schema, @name, @wdm:Schema, $keyrefName, 0)" />
						<mx:Binding source="{codegen:getClassNameClient(@name, @wdm:Schema)}(editor{$relationName}.selectedItem)" destination="_model.{$relationName}" />
					</xsl:if>
				</xsl:for-each>
        </xsl:for-each>
