...
@interface PhotoRSOViewController (MediaProcessing) <AVCaptureVideoDataOutputSampleBufferDelegate>
-(void)setupAVCapture;
...
@implementation PhotoRSOViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.title = @"Photo RSO";
        self.tabBarItem.image = [UIImage imageNamed:@"photorso"];
    }
    return self;
}
...
-(void)imageSaved:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    NSLog(@"-> imageSaved: error = %@ <%@>", error.code, error.localizedDescription);
}
...
-(void)disconnectedEvent {
    ...
    [self performSelector:@selector(doDisconnect) withObject:nil afterDelay:0.1f];
 	...
 	[self showAlert:[NSString stringWithString:@" !!! disconnectedEvent \n"]];   
}
...
@implementation PhotoRSOViewController (MediaProcessing)
...
-(void)takePicture {
    ...
    [stillImageOutput captureStillImageAsynchronouslyFromConnection:stillImageConnection
                                                  completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
                                                      if (error) {
                                                          NSLog(@"-> takePicture: error = %@ <%@>", error.code, error.localizedDescription);
                                                      }
                                                  }
 	 ];
}
