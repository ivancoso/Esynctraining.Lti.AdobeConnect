...
{
	if (eventName && handler && contextObject && !this._hasHandler( eventName, handler, contextObject ) )
	{
		var entry = this._eventMap[ eventName ];
		if ( entry == null )
...
DataMapperProxy.prototype._prepareCollection = function( dAsync, activeCollection, options )
{
	...
	dAsync.addResponder( new Responder(
		...
		function( e ) { alert( "Error loading data: " + e.description ); activeCollection.setIsLoading( false ); activeCollection.dispatchEvent( new ErrorEvent( e ) ); }
		)
	);
...
DataMapper.prototype.findAll = function()
{
	...
	else
	{
		//	sync call
		var dAsync = new DatabaseAsync();
		responder = dAsync.wrapAsync( new Async() );
		remoteObject.findAll( options, responder );
...
DataMapper.prototype.findBySql = function( sqlQuery, args )
{
	...
	else
	{
		...
		var dAsync = new DatabaseAsync();
		responder = dAsync.wrapAsync( new Async() );
		remoteObject.findBySql( sqlQuery, options, responder );
		return this._prepareCollection( dAsync, null, options );
	}
}
...
DatabaseAsync.prototype.wrapAsync = function( async, data, callbacksOnly )
{
	...
	return callbacksOnly? 
		new Async( this._execCallbacks, this._onFault, this ) 
...
//	================================================	ColumnInfo
function ColumnInfo( name, type, isPK, isRequired, isAutoIncremented )
{
...
ColumnInfo.defaultValues = { string: "", int: 0, float: 0, date: new Date(), boolean: false, Number : 0 };
...
ActiveRecord.prototype.equals = function( activeRecord )
{
	for( var i = 0; i < this._columnsInfo.length; ++i )
	{
		...
		var a = this[ n ], b = activeRecord[ n ];
		if( (a == null) != (b == null) || ( a != null && a.valueOf() !== b.valueOf() ) )
			return false;
	}	
	return true;
}
...
ActiveCollection.prototype.getItemAt = ActiveCollection.prototype.get = function( index )	//,	prefetch )
{
	if( this._paged )
	{
		...
		{
      return this._loadRecord(pageNumber);
    }
    
    if(this._isLoaded && this.getLength() == 0)
      return undefined; 

    var item = Collection.prototype.get.call( this, index );
    if(typeof item == "undefined")
    {
       var index = this._loadedPages.indexOf(pageNumber);
       this._loadedPages.splice(index, 1);
       item = this._loadRecord(pageNumber);
    }
...
ActiveCollection.prototype._loadRecord = function(pageNumber)
{
...
{
	delete this._affectedObjects[ activeRecord.ActiveRecordUID ];
	this._operationsCount--;
	this.dispatchEvent( new Event( "updated" ) );
}
...
{
	return this._affectedObjects[ activeRecord.ActiveRecordUID ] != null;
}
...
{
	Event.call( this, ErrorEvent.ERROR );
	this.data = data;
}

Utils.setInheritance( ErrorEvent, Event );

ErrorEvent.ERROR = "error";
//	endof ErrorEvent class
...
DataServiceClient.prototype.subscribe = function( clientId )
{
	...
	webORB.setDefaultRemotingURL(webOrbURL);
	
	var consumerConnectionUrl = webOrbURL;
	
	if(webORB.defaultMessagingURL)
		consumerConnectionUrl = webORB.defaultMessagingURL;
		
	this._consumer = new Consumer( 
		"DataService", 
		new Async( function(obj) { this.onMessage( obj ); }, function() { }, this ),
		consumerConnectionUrl,
		clientId,
		"clientId <> '" + clientId + "'");
	...
	this._consumer.subscribe( null, 
		new Async( 
			function( obj )
			{
				var remote = DataServiceClient.Instance.createRemoteObject( 
					webORB.defaultRemotingURL.indexOf( "/weborb.wo" ) >= 0? "weborb.data.management.DataServiceClientRegistry" : "Weborb.Data.Management.DataServiceClientRegistry" );
