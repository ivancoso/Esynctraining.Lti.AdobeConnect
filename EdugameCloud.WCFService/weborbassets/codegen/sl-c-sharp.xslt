...
namespace Invoker
{
    ...
    {
        const string CLASS_NAME = "<xsl:value-of select="concat(//service[ 1 ]/@namespace, '.', //service[ 1 ]/@name)"/>Service";
...
namespace Invoker
{
    public class ServiceModel
    {
		...
		{
			page.Dispatcher.BeginInvoke(() => System.Windows.Browser.HtmlPage.Window.Alert( " in fault - " + fault.Message ));
		}
  ...
  <xsl:template name="codegen.sl.vo">
    <file name="{@name}.cs">
      ...
      <xsl:for-each select="//enum[not(preceding-sibling::datatype/@typeNamespace=@typeNamespace or //datatype/@typeNamespace=@typeNamespace or @typeNamespace = current()/@typeNamespace)]">
       ...
       namespace <xsl:value-of select="@typeNamespace" />
        {
        public class <xsl:value-of select="@name"/><xsl:value-of select="@genericParameters"/><xsl:if test="@parentName"> : <xsl:value-of select="@parentNamespace"/>.<xsl:value-of select="@parentName"/></xsl:if>
        {
...
namespace <xsl:value-of select="@namespace" />
{
  public class <xsl:value-of select="@name"/>Service
  {
    ...
    <xsl:for-each select="method">
    public AsyncToken&lt;<xsl:choose><xsl:when test="@type = 'void'">Object</xsl:when><xsl:otherwise><xsl:value-of select="@nativetype"/></xsl:otherwise></xsl:choose>&gt;<xsl:text> </xsl:text><xsl:value-of select="@name"/>( <xsl:for-each select="arg"><xsl:value-of select="@nativetype" /><xsl:text> </xsl:text><xsl:value-of select="@name"/><xsl:if test="position() != last()">,</xsl:if><xsl:text> </xsl:text></xsl:for-each> )
    {
      return <xsl:value-of select="@name"/>(<xsl:for-each select="arg"><xsl:value-of select="@name"/>, </xsl:for-each> new Responder&lt;<xsl:choose><xsl:when test="@type = 'void'">Object</xsl:when><xsl:otherwise><xsl:value-of select="@nativetype"/></xsl:otherwise></xsl:choose>&gt;( <xsl:value-of select="@name"/>ResultHandler, null) );
    }
    ...
    namespace <xsl:value-of select="@namespace" />
    {
      ...
      {<xsl:for-each select="method">
        <xsl:choose>
            ...
            <xsl:otherwise>
                AsyncToken&lt;object&gt; <xsl:value-of select="@name"/>(<xsl:for-each select="arg"><xsl:value-of select="concat(@nativetype, ' ')" /> <xsl:value-of select="@name"/><xsl:if test="position() != last()">,</xsl:if></xsl:for-each>);
            </xsl:otherwise>
    </xsl:choose>
    </xsl:for-each>}
  } 
