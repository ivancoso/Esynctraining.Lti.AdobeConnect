      ...
      <mx:VBox xmlns:mx="http://www.adobe.com/2006/mxml" width="100%" height="100%" label="Products">
        ...
        <mx:columns>
          ...
          <xsl:for-each select="xs:complexType/xs:attribute[not(concat('@',@name) = key('fk',$class-name)/@xpath)]">
            <mx:DataGridColumn headerText="{codegen:getProperty($table,$schema,@name)}" dataField="{codegen:getProperty($table,$schema,@name)}">
    ...
    <xsl:for-each select="/xs:schema/xs:element">
      <xsl:choose>
        <xsl:when test="codegen:CountSchemas(@name) = 1">
          xmlns:<xsl:value-of select="codegen:FirstLatterToUpper(@wdm:Schema)" />="UI.TestDrive.*"
        </xsl:when>
        <xsl:otherwise>
          xmlns:<xsl:value-of select="codegen:FirstLatterToUpper(@wdm:Schema)" />="UI.TestDrive.<xsl:value-of select="@wdm:Schema" />.*"
        </xsl:otherwise>
      </xsl:choose>      
    </xsl:for-each>
      >
      ...
      <xsl:for-each select="/xs:schema/xs:element">
       ...
       &lt;<xsl:value-of select="codegen:FirstLatterToUpper(@wdm:Schema)" />:<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />DbView label="<xsl:value-of select="codegen:FirstLatterToUpper(@wdm:Schema)" />" width="100%" height="100%" top="20" bottom="20" left="20" right="20" /&gt;   
      </xsl:for-each>
      &lt;/mx:TabNavigator>
    &lt;/mx:Application>      
    </file>
