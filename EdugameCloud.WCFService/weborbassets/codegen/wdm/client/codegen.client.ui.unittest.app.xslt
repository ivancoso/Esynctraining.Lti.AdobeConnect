      ...
      <mx:Application xmlns:mx="http://www.adobe.com/2006/mxml" layout="absolute" xmlns:ns1="weborb.utest.*" creationComplete="onCreationComplete()">
        <mx:Script>
          ...
          <xsl:for-each select="/xs:schema/xs:element/xs:complexType/xs:choice/xs:element[@wdm:DatabaseObjectType='table']">
            unitTestEngine.items.addItem(new UTest<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />Create());
            unitTestEngine.items.addItem(new UTest<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />Update());
            unitTestEngine.items.addItem(new UTest<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />FindLast());
            unitTestEngine.items.addItem(new UTest<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />FindFirst());
            unitTestEngine.items.addItem(new UTest<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />Delete());
          </xsl:for-each>
