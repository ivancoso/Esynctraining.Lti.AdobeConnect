...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.oracle.create">
    ...
    {
    using(OracleCommand sqlCommand = Database.CreateCommand( SqlCreate ))
    {
    ...
    <xsl:for-each select="xs:complexType/xs:attribute[not(@msdata:AutoIncrement='true') and codegen:IsEditable(@type)]">
          ...
          <xsl:choose>
            <xsl:when test="$identity">
              <xsl:value-of select="$functionParam" />.<xsl:value-of select="codegen:getPropertyName($table, $schema, xs:complexType/xs:attribute[@msdata:AutoIncrement='true']/@name)"/> = <xsl:value-of select="codegen:CSharpDataType(xs:complexType/xs:attribute[@msdata:AutoIncrement='true']/@type)" />.Parse( sqlCommand.ExecuteScalar().ToString()) ;
            </xsl:when>
            <xsl:otherwise>
