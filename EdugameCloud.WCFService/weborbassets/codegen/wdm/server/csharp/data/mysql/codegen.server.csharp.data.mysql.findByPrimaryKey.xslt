...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.mysql.findByPrimaryKey">
    ...
    )
    {
      ...
      {
          using(MySqlCommand sqlCommand = Database.CreateCommand(SqlSelectByPk))
          {
            <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
              sqlCommand.Parameters.AddWithValue("?<xsl:value-of select="@name"/>", <xsl:value-of select="codegen:FunctionParameter(@name)" />);
            </xsl:for-each>
    ...
    public bool exists(<xsl:value-of select="$class-name" /> <xsl:text> </xsl:text><xsl:value-of select="$functionParam" />)
    {
      using (DatabaseConnectionMonitor monitor = new DatabaseConnectionMonitor(Database))
      {
          ...
          {
              <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
                sqlCommand.Parameters.AddWithValue("?<xsl:value-of select="@name"/>", <xsl:value-of select="$functionParam" />.<xsl:value-of select="codegen:getPropertyName($table,$schema,@name)" />);
              </xsl:for-each>
    ...
    protected override IDbCommand prepareCheckInCommand(DomainObject domainObject, string sqlQuery)
    {
      <xsl:value-of select="$class-name"/> _<xsl:value-of select="$class-name"/> = (<xsl:value-of select="$class-name"/>)domainObject;
       ...
       MySqlCommand sqlCommand = Database.CreateCommand(modifyQueryForCheckIn(sqlQuery,CheckInSql));
