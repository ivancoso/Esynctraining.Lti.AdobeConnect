        ...
        List&lt;<xsl:value-of select="$class-name" />&gt; resultList = new List&lt;<xsl:value-of select="$class-name" />&gt;();
         ...
         using(MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(sqlCommand))
         {
            ...
            foreach(DataRow dataRow in dataTable.Rows)
            {
              ...
              <xsl:for-each select="xs:complexType/xs:attribute">
                <xsl:variable name="property" select="codegen:getProperty($table,$schema,@name)" />
                <xsl:variable name="csType" select="codegen:CSharpDataType(@type)" />
                <xsl:if test="@use = 'optional'">
                  if(!dataRow.IsNull("<xsl:value-of select="@name"/>"))
                </xsl:if>
                <xsl:choose>
                  <xsl:when test="@type = 'xs:base64Binary'">
                    item.<xsl:value-of select="$property"/> = dataRow["<xsl:value-of select="@name"/>"] as <xsl:value-of select="$csType" />;
                  </xsl:when>
                  <!-- MYSQL .NET PROVIDER VERSION 6.3.6 RETURNS BOOLEAN, NO NEED FOR THIS CHECK
                  <xsl:when test="@type = 'xs:boolean'">
                    item.<xsl:value-of select="$property"/> = ((sbyte)dataRow["<xsl:value-of select="@name"/>"]) == 1;
                  </xsl:when>
                  -->
                  <xsl:otherwise>
                    item.<xsl:value-of select="$property"/> = <xsl:choose><xsl:when test="$csType = 'bool'">dataRow["<xsl:value-of select="@name"/>"].ToString() == "0" ? false : true</xsl:when><xsl:otherwise>( <xsl:value-of select="$csType" />)dataRow["<xsl:value-of select="@name"/>"]</xsl:otherwise></xsl:choose>;
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:for-each>
