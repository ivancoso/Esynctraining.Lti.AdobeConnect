...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.odbc.findByPrimaryKey">
    ...
    <xsl:for-each select="xs:complexType/xs:attribute">
      <xsl:value-of select="codegen:quoteIdentifier(@name)" /> <xsl:if test="position() != last()">,</xsl:if> 
    </xsl:for-each>
     From <xsl:value-of select="codegen:quoteIdentifier(@wdm:Schema)" />.<xsl:value-of select="codegen:quoteIdentifier($table)" />
    ...
    {
    using(OdbcCommand sqlCommand = Database.CreateCommand(SqlSelectByPk))
    {
        <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
          sqlCommand.Parameters.AddWithValue("?", <xsl:value-of select="codegen:FunctionParameter(@name)" />);
        </xsl:for-each>
    ...
    public bool exists(<xsl:value-of select="$class-name" /> <xsl:text> </xsl:text><xsl:value-of select="$functionParam" />)
    {
      using (DatabaseConnectionMonitor monitor = new DatabaseConnectionMonitor(Database))
      {
          ...
          {
              <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
                sqlCommand.Parameters.AddWithValue("?", <xsl:value-of select="$functionParam" />.<xsl:value-of select="codegen:getPropertyName($table,$schema,@name)" />);
              </xsl:for-each>
    ...
    <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
      <xsl:value-of select="codegen:quoteIdentifier($table)"/>.<xsl:value-of select="codegen:quoteIdentifier(@name)" /> = @CheckIn<xsl:value-of select="@name" /> <xsl:if test="position() != last()"> and </xsl:if>
    </xsl:for-each>";
    ...
    protected override IDbCommand prepareCheckInCommand(DomainObject domainObject, string sqlQuery)
    {
      <xsl:value-of select="$class-name"/> _<xsl:value-of select="$class-name"/> = (<xsl:value-of select="$class-name"/>)domainObject;
       ...
       OdbcCommand sqlCommand = Database.CreateCommand(modifyQueryForCheckIn(sqlQuery,CheckInSql));

      <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
        sqlCommand.Parameters.AddWithValue("@CheckIn<xsl:value-of select="@name"/>", _<xsl:value-of select="$class-name"/>.<xsl:value-of select="codegen:getPropertyName($table,$schema,@name)" />);
      </xsl:for-each>
