...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.odbc.load">
    ...
    protected override List&lt;<xsl:value-of select="$class-name" />&gt; fill(OdbcCommand sqlCommand, int offset, int limit)
    {
         List&lt;<xsl:value-of select="$class-name" />&gt; resultList = new List&lt;<xsl:value-of select="$class-name" />&gt;();
    
         using(OdbcDataAdapter sqlDataAdapter = new OdbcDataAdapter(sqlCommand))
         {
    ...
    <xsl:for-each select="xs:keyref">
      ...
      private const String <xsl:value-of select="$sql-name"/> = @"select * from <xsl:value-of select="codegen:quoteIdentifier($schema)"/>.<xsl:value-of select="codegen:quoteIdentifier($table)" />
      where
      <xsl:for-each select="xs:field">
        <xsl:if test="position() != 1">
          and
        </xsl:if>
        <xsl:value-of select="codegen:quoteIdentifier(substring(@xpath,2))"/> = @<xsl:value-of select="substring(@xpath,2)"/>
      </xsl:for-each>";
      ...
      internal List&lt;<xsl:value-of select="$class-name" />&gt; findBy<xsl:value-of select="$parent-property"/>(<xsl:value-of select="codegen:getClassName($parent-table,$parent-schema)"/> domainObject, QueryOptions queryOptions)
      {
        ...
        using(OdbcCommand sqlCommand = Database.CreateCommand(<xsl:value-of select="$sql-name"/>))
        {
          <xsl:for-each select="xs:field">
