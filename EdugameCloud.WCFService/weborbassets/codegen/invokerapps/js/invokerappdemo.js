...
var entryMap = {};
function addToEntryMap(name, entry, model)
{
	entryMap[name] = { 'entry': entry, 'model': model};
}
function getEntryFromMap(name)
{
...
function objectEntryToHtml( objName, entry, entryDepth, entryParent) 
{
    ...
    addToEntryMap(objName, entry, modelObj);
	var addLink = "<a href='javascript:void(0)' onClick='instantiateObject(\""+objName+"\"," + arrayIDCounter + "," + (entryDepth + 1) +", this);' title=''>Instantiate</a>";
	
	var result = "<tr><td style='padding-left:" + ((entryDepth + 1) * 10) + "px'>"
			+ (isArray( entryParent ) ? "[" +objName+ "]" : objName) + "</td><td>Object</td><td>"+addLink+"</td></tr>";// style='background:#cecece'
	
	result += "</td></tr><tr id='instantiateObj" + arrayIDCounter + "' style='display:none'></tr>";
	arrayIDCounter++;
...
function instantiateObject(name, objectID, entryDepth, ui ){
	var placeHolderRow = document.getElementById( "instantiateObj" + objectID );
	ui.style.display = "none";
		
	var modelObj = getEntryFromMap(name).model;
	var entry = getEntryFromMap(name).entry;
	var result = '';
	if( propertyCounter( entry ) == 1 )
	{
		var className = entry.__woproto__;
