...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.vo.folder">
      <xsl:if test="count(enum) != 0 or count(datatype) != 0">
          <folder name="model">
	  ...
	  <xsl:param name="version" select="3" />
        ...
        <file name="DataTypeInitializer.as">
          <xsl:call-template name="codegen.datatypelist">
			...
			<xsl:with-param name="subnamespace" select="string('.model.vo.')" />
          </xsl:call-template>
        </file>
      </xsl:if>
...
package <xsl:value-of select="@typeNamespace"/>.model.vo
{
	...
	{	<xsl:for-each select="field">
		...
		{
		return <xsl:choose><xsl:when test="count(field) = 0">""</xsl:when><xsl:otherwise><xsl:for-each select="field">this.<xsl:value-of select="@name"/><xsl:if test="position()!=last()"> + ": " +</xsl:if>
...
<xsl:for-each select="//namespace/datatype">
	import <xsl:value-of select="@typeNamespace" />.model.vo.<xsl:value-of select="@name"/>;</xsl:for-each>
<xsl:for-each select="//namespace/enum">
	import <xsl:value-of select="@typeNamespace" />.model.enum.<xsl:value-of select="@name"/>;</xsl:for-each>  	
...
package <xsl:value-of select="../@namespace" />.controller
{
	...
	import <xsl:value-of select="../@namespace" />.model.<xsl:value-of select="parent::node()/@name"/>Proxy;
<xsl:for-each select="//namespace/datatype">
	import <xsl:value-of select="@typeNamespace" />.model.vo.<xsl:value-of select="@name"/>;</xsl:for-each>
<xsl:for-each select="//namespace/enum">
	import <xsl:value-of select="@typeNamespace" />.model.enum.<xsl:value-of select="@name"/>;</xsl:for-each>  	
...
package <xsl:value-of select="@fullname" />.model.vo
{<xsl:for-each select="//namespace/datatype">
	import <xsl:value-of select="@typeNamespace" />.model.vo.<xsl:value-of select="@name"/>;</xsl:for-each>
<xsl:for-each select="//namespace/enum">
