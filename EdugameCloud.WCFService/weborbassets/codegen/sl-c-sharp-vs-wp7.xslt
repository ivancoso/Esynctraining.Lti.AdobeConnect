...
namespace Invoker
{
  ...
  {
    internal const string CLASS_NAME = "<xsl:value-of select="concat(//service[ 1 ]/@namespace, '.', //service[ 1 ]/@name)"/>Service";
...
<Project ToolsVersion="4.0" DefaultTargets="Build" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  <PropertyGroup>
    ...
    <ProjectTypeGuids>{C089C8C0-30E0-4E22-80C0-CE093F111A43};{<xsl:value-of select="codegen:getGuidForVSSolution()"/>}</ProjectTypeGuids>
  ...
  <ItemGroup>
    ...
    <xsl:for-each select="//service">
      <xsl:variable name="sourceFilePath" select="translate(@namespace,'.','\')"/>
      <Compile Include="{$sourceFilePath}\{@name}Service.cs" />
      <Compile Include="{$sourceFilePath}\{@name}Model.cs" />
    </xsl:for-each>
    <xsl:for-each select="//datatype">
      <xsl:variable name="sourceFilePath" select="translate(@typeNamespace,'.','\')"/>
...
# Visual Studio 2010
Project("{<xsl:value-of select="codegen:getGuidForVSSolution()"/>}") = "InvokerApp", "InvokerDemoApp\InvokerApp.csproj", "{01C6E01F-7887-4598-AE7F-572B26EFF984}"
EndProject
Project("{<xsl:value-of select="codegen:getGuidForVSSolution()"/>}") = "RemoteServiceLibrary", "RemoteServiceLibrary\RemoteServiceLibrary.csproj", "{<xsl:value-of select="$projectGuid" />}"
EndProject
Global
