...
<xsl:if test='$version=3'>	public</xsl:if> class <xsl:choose>
  ...
  <xsl:otherwise>
	...
	<xsl:for-each select="field">
        public<xsl:call-template name="codegen.add.override.if.needed"/> var <xsl:value-of select="@name"/>:<xsl:choose><xsl:when test="@typeNamespace"><xsl:value-of select="@typeNamespace"/>.vo.<xsl:value-of select="@type"/></xsl:when><xsl:otherwise><xsl:value-of select="@fulltype"/></xsl:otherwise></xsl:choose>;</xsl:for-each>
	}<xsl:if test="$version=3">
}
</xsl:if>
</file>
...
<xsl:if test='$version=3'>  public</xsl:if> class <xsl:choose>
	  ...
	  </xsl:otherwise></xsl:choose>
	{
		public function <xsl:value-of select="@name"/>(){}
	<xsl:for-each select="field">
		public<xsl:call-template name="codegen.add.override.if.needed"/> static var <xsl:value-of select="@name"/>:String="<xsl:value-of select="@name"/>";</xsl:for-each>
	}<xsl:if test="$version=3">
}
</xsl:if>
</file>
