...
namespace Esynctraining.Lti.Lms.Common.API.Desire2Learn
{
    public interface IDesire2LearnApiService
    {
        ...
        T GetApiObjects<T>(Uri urlWithAuthParams, string hostUrl, string apiUrl,
            Dictionary<string, object> licenseSettings) where T : new();

        T GetApiObjects<T>(string userId, string userKey, string hostUrl, string apiUrl,
            Dictionary<string, object> licenseSettings) where T : new();
